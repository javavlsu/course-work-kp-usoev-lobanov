...
import { GET_CARS, GET_STATISTIC } from '../api/api';
import axios from 'axios';
import AlertDialog from './forms/dialog';
import StatisticDialog from './forms/statistic';
...
export default function Car() {
    ...
    const getStatistic = useCallback((key) => {
        axios
            .get(GET_STATISTIC, { params: { id: key } })
            .then((response) => {
                if (response.status === 200) {
                    console.log(response.data);
                    ...
                    {cars.map((row) => (
                        <Card sx={{ maxWidth: 345, margin: "1%" }} key={row.id}>
                            ...
                            <CardActions>
                                ...
                                <Button size="small" onClick={() => handleOpen(row.id)}>Подробности</Button>
                                <StatisticDialog open={openStatistic} handleClose={handleClose} cars={statistic} />
                            </CardActions>
